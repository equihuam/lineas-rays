...
# Ordenar los temas por relevancia de mayor a menor
sorted_relevance = df_updated[["Tema", "Relevancia"]].sort_values(by="Relevancia", ascending=False).reset_index(drop=True)
 
show(sorted_relevance)
```
...
sorted_originalidad = df_updated[["Tema", "Originalidad"]].sort_values(by="Originalidad", ascending=False).reset_index(drop=True)

show(sorted_originalidad)
...
df_updated["Relevancia_jitter"] = df_updated["Relevancia"] + np.random.uniform(-jitter_strength, jitter_strength, size=len(df_updated))
df_updated["Originalidad_jitter"] = df_updated["Originalidad"] + np.random.uniform(-jitter_strength, jitter_strength, size=len(df_updated))
...
vectorizer = TfidfVectorizer()
X = vectorizer.fit_transform(temas)
...
plt.title("Dendrograma de temas (etiquetas abreviadas)")
plt.tight_layout()
plt.show()

# Opcional: mostrar leyenda
leyenda = pd.DataFrame({
    "Etiqueta Corta": etiquetas_cortas,
    "Tema Completo": temas
})
...
tema_actor_clave_data = {
    "Cambio global": ["Gubernamentales", "Académicos", "Jóvenes"],
    "Conservación comunitaria": ["Indígenas", "Comunidades rurales", "Académicos", "Mujeres"],
    "Evaluación ambiental": ["Gubernamentales", "Comunidades rurales", "Académicos"],
    "Estudios ocioecológicos": ["Jóvenes", "Mujeres", "Académicos", "Indígenas"],
    "Gestión del agua": ["Gubernamentales", "Comunidades rurales", "Indígenas", "Académicos"],
    "Justicia socioambiental": ["Gubernamentales", "Indígenas", "Mujeres", "Académicos", "Jóvenes"],
    "Ecología de insectos": ["Académicos", "Gubernamentales"],
    "Monitoreo ambiental": ["Gubernamentales", "Académicos", "Jóvenes"]
}
...
nx.draw_networkx_labels(G_vinculos, pos, font_size=12)

plt.title("Gráfico relacional: Temas y actores clave", fontsize=14 )
plt.axis('off')
plt.tight_layout()
plt.show()
...
nx.draw_networkx_labels(G_nuevos, pos, font_size=12)

plt.title("Gráfico relacional: Temas y nuevos actores clave", fontsize=14      )
plt.axis('off')
#plt.tight_layout()
plt.show()
